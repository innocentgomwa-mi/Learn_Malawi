...
import { fetchInsights, fetchAttendanceHistory } from '@/api';
...
export default function TeacherAnalytics() {
  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      ...
      try {
        try {
        const [insightData, attendanceData] = await Promise.all([
          fetchInsights({ limit: 1000 }),
          fetchAttendanceHistory(user.email),
        ]);
        const supportedAttempts = Array.isArray(insightData?.quizAttempts) ? insightData.quizAttempts : [];
        const supportedAttendance = Array.isArray(attendanceData) ? attendanceData : [];
        setQuizAttempts(filterByTeacher(supportedAttempts, user.email));
        const groupedQuizzes = Object.values(
          supportedAttempts.reduce((acc, attempt) => {
            const key = `${attempt.quiz_id || attempt.quiz_title || attempt.subject || 'quiz'}`;
            if (!acc[key]) {
              acc[key] = {
                id: attempt.quiz_id || key,
                title: attempt.quiz_title || attempt.subject || 'Untitled Quiz',
                subject: attempt.subject || 'General',
                class: attempt.level || '',
                questions: [{}, {}], // placeholder for compatibility (length-based)
                attempts: 0,
                avgScore: 0,
                scores: [],
              };
            ...
            acc[key].attempts += 1;
            acc[key].scores.push(Number(attempt.score_percentage || 0));
            return acc;
          }, {}),
        ).map((quiz) => ({
          ...quiz,
          avgScore: quiz.scores.length ? Math.round(quiz.scores.reduce((a, b) => a + b, 0) / quiz.scores.length) : 0,
          questions: Array(Math.max(quiz.scores.length, 1)).fill({}),
        }));
  ...
  const filteredQuizAttempts = selectedClass === ALL_CLASSES_VALUE
    ? quizAttempts
    : quizAttempts.filter((attempt) => (attempt?.level || attempt?.class || '') === selectedClass);
  const averageQuizScore = filteredQuizAttempts.length > 0
    ? Math.round(
        filteredQuizAttempts.reduce((sum, attempt) => {
          const score = Number(attempt?.score_percentage ?? attempt?.score ?? 0);
          return sum + (Number.isFinite(score) ? score : 0);
        }, 0) / filteredQuizAttempts.length,
      )
    : 0;
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Quiz Attempts', value: filteredQuizAttempts.length, color: 'text-sky-600' },
              { label: 'Avg Quiz Score', value: `${averageQuizScore}%`, color: 'text-emerald-600' },
