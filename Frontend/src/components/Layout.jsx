  ...
  return (
    <div className="min-h-screen flex flex-col">
      {!hideTopNav && (
        <>
          {/* Top Nav */}
          <header className="bg-primary text-primary-foreground sticky top-0 z-40 shadow-lg" aria-label="Site header">
            <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <ThemeToggle />
                {isAuthenticated && (
                  <Link to="/dashboard" className={`p-2 rounded-lg hover:bg-primary-foreground/10 text-primary-foreground hidden md:block ${location.pathname === '/dashboard' ? 'bg-secondary text-secondary-foreground' : ''}`} aria-label="My Dashboard">
                    <User className="h-5 w-5" />
                  </Link>
                )}
                {['admin', 'teacher'].includes(user?.role?.toLowerCase() ?? '') && (
                  <Link to="/teacher" className={`p-2 rounded-lg hover:bg-primary-foreground/10 text-primary-foreground hidden md:block ${location.pathname.startsWith('/teacher') ? 'bg-secondary text-secondary-foreground' : ''}`} aria-label="Teacher dashboard">
            ...
            {/* Mobile Menu */}
            {mobileOpen && (
              <div id="mobile-menu" className="md:hidden border-t border-primary-foreground/20 bg-primary px-4 pb-4" role="navigation" aria-label="Mobile navigation">
                {navItems
                  .filter((item) => (!item.authRequired || isAuthenticated) && (!item.guestOnly || !isAuthenticated))
                  .map(({ path, label, icon: Icon }) => (
                    <Link
      ...
      {/* Page Content */}
      <main id="main-content" className="flex-1" aria-label="Page content">
        <Outlet />
      </main>

      {/* Footer */}
      <footer className="bg-primary text-primary-foreground/70 py-6 text-center text-sm mt-8" aria-label="Site footer">
