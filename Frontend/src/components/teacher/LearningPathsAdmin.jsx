...
function MilestoneEditor({ milestones = [], resourceOptions = [], loadingResources = false, onChange, onCreateQuiz }) {
  ...
  const toggleResource = (i, resourceId) => onChange(currentMilestones.map((m, idx) => {
    ...
    const selected = (Array.isArray(m.resource_ids) ? m.resource_ids : []).map((id) => String(id));
    return {
      ...m,
      resource_ids: selected.includes(normalizedId)
                ...
                return (
                  <div key={`${resource.type}-${resource.id}`} className="flex items-center gap-2">
                    <label className="flex items-center gap-2 cursor-pointer flex-1">
                      <input
                        type="checkbox"
                        checked={(m.resource_ids || []).map((id) => String(id)).includes(String(resource.id))}
...
export default function LearningPathsAdmin() {
  ...
  const assignQuizToMilestone = (quiz, milestoneIndex) => {
    ...
    const nextMilestones = (form?.data?.milestones || []).map((milestone, index) => {
      ...
      const selectedIds = Array.isArray(milestone.resource_ids) ? milestone.resource_ids.map((id) => String(id)) : [];
      return {
        ...milestone,
        title: milestone.title || quiz.title || milestone.title,
        resource_ids: Array.from(new Set([...selectedIds, quizId])),
      };
    });
  ...
  const cleanMilestonesForSave = (milestones = []) => {
    return milestones.map(({ id, ...rest }) => ({
      ...
      resource_ids: Array.isArray(rest.resource_ids)
        ? rest.resource_ids.map((value) => String(value)).filter(Boolean)
        : [],
      order: rest.order ?? 0,
    }));
  };
