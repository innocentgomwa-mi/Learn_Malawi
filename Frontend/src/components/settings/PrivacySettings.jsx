...
export default function PrivacySettings() {
  ...
  useEffect(() => {
    ...
    setDataTracking(user.allowDataTracking ?? true);
  }, [user]);
  ...
  const handleSave = async () => {
    ...
    try {
      await updateProfile({
        ...
        allowDataTracking: Boolean(dataTracking),
      });
      await refreshUser();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      <SettingsSection title="Profile Privacy">
        ...
        <div className="pt-2">
          <Button onClick={handleSave} disabled={saving} className="bg-yellow-400 text-blue-950 hover:bg-yellow-300">
