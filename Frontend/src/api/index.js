...
async function request(path, options = {}) {
  ...
  try {
    ...
    throw new Error(`Network error: ${message || 'Unable to reach the server'}`);
  }
...
export async function authVerifyTwoFactor(challengeId, code) {
...
export async function authRequestEnableTwoFactor() {
  return request('/auth/two-factor/enable/request', { method: 'POST' });
}

export async function authConfirmEnableTwoFactor(code) {
  return request('/auth/two-factor/enable/confirm', {
    method: 'POST',
    body: JSON.stringify({ code }),
  });
}

export async function authRequestDisableTwoFactor() {
  return request('/auth/two-factor/disable/request', { method: 'POST' });
}

export async function authConfirmDisableTwoFactor(code) {
  return request('/auth/two-factor/disable/confirm', {
    method: 'POST',
    body: JSON.stringify({ code }),
  });
}
...
export function fetchStudyNotes({ level, subject, search, teacherEmail } = {}) {
  ...
  const query = params.toString() ? `?${params.toString()}` : '';
  return request(`/study-notes${query}`).then((response) => {
...
export function fetchResourceCount() {
  return request('/resources/count');
}
...
export function fetchChatMessages({ room, notificationFeed } = {}) {
  const params = new URLSearchParams();
  if (room) params.set('room', room);
  if (notificationFeed) params.set('notification_feed', 'true');
  const query = params.toString() ? `?${params.toString()}` : '';
...
export function fetchStudentCount() {
  return request('/users/students/count');
}

export function fetchQuizCount() {
  return request('/quizzes/count');
}
...
export function uploadStudyGroupFile(formData) {
  return request('/shared-resources/upload', { method: 'POST', body: formData });
}

export function fetchStudyGroupResources({ search } = {}) {
  const params = new URLSearchParams();
  if (search?.trim()) params.set('search', search.trim());
  const query = params.toString() ? `?${params.toString()}` : '';
  return request(`/shared-resources${query}`);
...
export function updateStudyGroup(id, data) {
  return request(`/study-groups/${id}`, {
    method: 'PATCH',
...
export function removeStudyGroupMember(groupId, memberEmail, { ban = false } = {}) {
  const encoded = encodeURIComponent(memberEmail);
  const query = ban ? '?ban=true' : '';
  return request(`/study-groups/${groupId}/members/${encoded}${query}`, { method: 'DELETE' });
}
...
export function fetchTutorials({ teacherEmail, level, subject, classFilter, search } = {}) {
  const params = new URLSearchParams();
  if (teacherEmail) params.set('teacher_email', teacherEmail);
  if (level) params.set('level', level);
  if (subject) params.set('subject', subject);
  ...
  if (search?.trim()) params.set('search', search.trim());
  const query = params.toString() ? `?${params.toString()}` : '';
  return request(`/tutorials${query}`).then((response) => {
    if (Array.isArray(response)) return response;
    return response?.data ?? [];
  });
}
...
export function createCareerResource(data) {
  return request('/career-resources', { method: 'POST', body: isFormData(data) ? data : JSON.stringify(data) });
}
...
export function updateCareerResource(id, data) {
  return request(`/career-resources/${id}`, { method: 'PATCH', body: isFormData(data) ? data : JSON.stringify(data) });
}
 ...
 */
export function fetchQuizzes({ level, subject, difficulty, classFilter, teacherEmail, search } = {}) {
  const params = new URLSearchParams();
  if (level && level !== 'All') params.set('level', level);
  if (subject) params.set('subject', subject);
  ...
  if (search?.trim()) params.set('search', search.trim());
  const query = params.toString() ? `?${params.toString()}` : '';
  return request(`/quizzes${query}`).then((response) => {
    if (Array.isArray(response)) return response;
    return response?.data ?? [];
  });
}
