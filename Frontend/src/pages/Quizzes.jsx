...
export default function Quizzes() {
  ...
  const { data: quizzes = [], isLoading: loading } = queryResult;
  ...
  const startQuiz = (quiz) => { setActiveQuiz(quiz); setAnswers({}); setSubmitted(false); setAiFeedback(""); };
  ...
  const generateSelfTest = async () => {
    ...
    try {
      ...
      const questions = generated.map((q) => {
        ...
        const cleanOptions = rawOptions.map((o) => typeof o === "string" ? o.replace(/^[A-D]\)\s*/i, "").trim() : o);
        ...
        <div className="bg-card border border-border rounded-2xl p-6">
          ...
          {!submitted ? (
            <button onClick={submitQuiz}
              disabled={Object.keys(answers).filter(k => answers[k]).length < questions.length}
