...
export class TutorialsService {
  ...
  async findAll(
    ...
    if (subject) {
      query.andWhere('tutorial.subject ILIKE :subject', { subject: `%${subject}%` });
    }

    if (classFilter) {
      query.andWhere('tutorial.class ILIKE :classFilter', { classFilter: `%${classFilter}%` });
    }
    ...
    if (trimmedSearch) {
      query.andWhere(
        `(tutorial.title ILIKE :search
          OR tutorial.subject ILIKE :search
          OR tutorial.description ILIKE :search
          OR tutorial.class ILIKE :search)`,
        { search: `%${trimmedSearch}%` },
      );
    }

    query.orderBy('tutorial.createdAt', 'DESC');
