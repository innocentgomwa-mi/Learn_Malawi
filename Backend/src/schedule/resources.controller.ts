...
export class ResourcesController {
  ...
  @Get('count')
  count(@User() user: any) {
    return this.scheduleService.countResources(user?.email);
  }
  ...
  @Get(':id')
  findOne(@Param('id', new ParseUUIDPipe({ version: '4' })) id: string, @User() user: any) {
    return this.scheduleService.findResource(id, user?.email);
  }

  @Patch(':id')
  update(@Param('id', new ParseUUIDPipe({ version: '4' })) id: string, @User() user: any, @Body() updateResourceDto: UpdateResourceDto) {
    return this.scheduleService.updateResource(id, updateResourceDto, user?.email);
  }

  @Delete(':id')
  remove(@Param('id', new ParseUUIDPipe({ version: '4' })) id: string, @User() user: any) {
