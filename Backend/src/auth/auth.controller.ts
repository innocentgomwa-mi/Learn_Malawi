...
export class AuthController {
  ...
  async verifyTwoFactor(@Body() dto: TwoFactorVerifyDto): Promise<TokenResponseDto> {
    return this.authService.verifyTwoFactorLogin(dto.challengeId, dto.code);
  }
  ...
  @Post('two-factor/enable/request')
  @UseGuards(JwtAuthGuard)
  async requestEnable2fa(@Request() req): Promise<{ message: string }> {
    return this.authService.requestEnableTwoFactor(req.user.id);
  }

  @Post('two-factor/enable/confirm')
  @UseGuards(JwtAuthGuard)
  async confirmEnable2fa(@Request() req, @Body() dto: TwoFactorConfirmDto): Promise<{ message: string }> {
    return this.authService.confirmEnableTwoFactor(req.user.id, dto.code);
  }

  @Post('two-factor/disable/request')
  @UseGuards(JwtAuthGuard)
  async requestDisable2fa(@Request() req): Promise<{ message: string }> {
    return this.authService.requestDisableTwoFactor(req.user.id);
  }

  @Post('two-factor/disable/confirm')
  @UseGuards(JwtAuthGuard)
  async confirmDisable2fa(@Request() req, @Body() dto: TwoFactorConfirmDto): Promise<{ message: string }> {
    return this.authService.confirmDisableTwoFactor(req.user.id, dto.code);
  }
