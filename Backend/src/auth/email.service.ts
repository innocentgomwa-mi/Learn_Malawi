...
export class EmailService {
  ...
  async sendVerificationCode(email: string, code: string) {
    ...
    const message = `Hello,\n\nYour ${appName} verification code is ${code}. It will expire in 5 minutes.\n\nIf you did not request this code, please ignore this email.\n\nThank you,\n${appName} team`;
  ...
  async sendPasswordResetCode(email: string, code: string) {
    ...
    const message = `Hello,\n\nYour ${appName} password reset code is ${code}. It will expire in 5 minutes.\n\nIf you did not request this code, please ignore this email or contact support.\n\nThank you,\n${appName} team`;
  ...
  async sendTwoFactorCode(email: string, code: string) {
    const appName = this.configService.get<string>('APP_NAME') || 'Learn Malawi';
    const subject = `${appName} security code`;
    const message = `Hello,\n\nYour ${appName} security code is ${code}. It will expire in 5 minutes.\n\nIf you did not request this code, please ignore this email.\n\nThank you,\n${appName} team`;
    await this.sendMessage(email, subject, message);
  }
