...
export class AuthService {
  ...
  constructor(
    ...
    private twoFactorChallengeRepository: Repository<TwoFactorChallenge>,
  ...
  async login(loginDto: LoginDto): Promise<TokenResponseDto> {
    ...
    if (user?.twoFactorEnabled) {
      const challenge = await this.createTwoFactorChallenge(user.id, 'login');
      try {
        await this.emailService.sendTwoFactorCode(user.email, challenge.code);
      } catch {
        // ignore email failures (code still created)
      }
      return {
  ...
  async verifyTwoFactorLogin(challengeId: string, code: string): Promise<TokenResponseDto> {
    const challenge = await this.verifyTwoFactorChallenge(challengeId, code, 'login');
    const user = await this.usersService.findOne(challenge.userId);
  ...
  async requestEnableTwoFactor(userId: string): Promise<{ message: string }> {
    const user = await this.usersService.findOne(userId);
    const challenge = await this.createTwoFactorChallenge(userId, 'enable');
    try {
      await this.emailService.sendTwoFactorCode(user.email, challenge.code);
    } catch (error) {
      this.logger.warn(`2FA enable code email failed for ${user.email}: ${(error as Error)?.message || error}`);
    }
    return { message: 'Two-factor enable code sent to your email.' };
  }

  async confirmEnableTwoFactor(userId: string, code: string): Promise<{ message: string }> {
    const latest = await this.findLatestChallenge(userId, 'enable');
    await this.verifyTwoFactorChallenge(latest.id, code, 'enable');
    await this.usersService.update(userId, { twoFactorEnabled: true } as any);
    return { message: 'Two-factor authentication enabled.' };
  }

  async requestDisableTwoFactor(userId: string): Promise<{ message: string }> {
    const user = await this.usersService.findOne(userId);
    const challenge = await this.createTwoFactorChallenge(userId, 'disable');
    try {
      await this.emailService.sendTwoFactorCode(user.email, challenge.code);
    } catch (error) {
      this.logger.warn(`2FA disable code email failed for ${user.email}: ${(error as Error)?.message || error}`);
    }
    return { message: 'Two-factor disable code sent to your email.' };
  }

  async confirmDisableTwoFactor(userId: string, code: string): Promise<{ message: string }> {
    const latest = await this.findLatestChallenge(userId, 'disable');
    await this.verifyTwoFactorChallenge(latest.id, code, 'disable');
    await this.usersService.update(userId, { twoFactorEnabled: false } as any);
    return { message: 'Two-factor authentication disabled.' };
  }

  private async createTwoFactorChallenge(userId: string, purpose: TwoFactorPurpose) {
    const code = this.generateVerificationCode();
    const codeHash = await bcrypt.hash(code, 10);
    const expiresAt = new Date(Date.now() + 5 * 60 * 1000);
    const entity = this.twoFactorChallengeRepository.create({
      userId,
      purpose,
      codeHash,
      expiresAt,
    ...
    const saved = await this.twoFactorChallengeRepository.save(entity);
    return { id: saved.id, code };
  }

  private async findLatestChallenge(userId: string, purpose: TwoFactorPurpose): Promise<TwoFactorChallenge> {
    const latest = await this.twoFactorChallengeRepository.findOne({
      where: { userId, purpose, used: false },
      order: { createdAt: 'DESC' as any },
    });
    if (!latest) {
      throw new BadRequestException('No active security code found. Please request a new code.');
    }
    return latest;
  }

  private async verifyTwoFactorChallenge(challengeId: string, code: string, purpose: TwoFactorPurpose): Promise<TwoFactorChallenge> {
    const challenge = await this.twoFactorChallengeRepository.findOne({ where: { id: challengeId } });
    if (!challenge || challenge.used || challenge.purpose !== purpose) {
      throw new UnauthorizedException('Invalid security code.');
    }
    if (challenge.expiresAt < new Date()) {
      throw new UnauthorizedException('Security code expired.');
    }
  ...
  async verifyEmail(verifyEmailDto: VerifyEmailDto): Promise<{ message: string }> {
    ...
    const existingUser = await this.usersService.findOneByEmail(pending.email);
    if (existingUser) {
