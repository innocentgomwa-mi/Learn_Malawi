...
export class UsersController {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(UserRole.ADMIN, UserRole.TEACHER, UserRole.STUDENT)
  async countStudents(): Promise<{ count: number }> {
    const count = await this.usersService.countUsersByRole(UserRole.STUDENT);
    return { count };
  }
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(UserRole.ADMIN)
  async remove(@Request() req, @Param('id', new ParseUUIDPipe({ version: '4' })) id: string): Promise<{ message: string }> {
