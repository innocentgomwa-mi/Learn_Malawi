...
const refreshAccessToken = async () => {
  const refreshToken = typeof window !== 'undefined' ? window.sessionStorage.getItem(REFRESH_TOKEN_KEY) : null;
  if (!refreshToken) {
...
const buildHeaders = (extraHeaders = {}, body) => {
  const headers = {
    ...extraHeaders,
  };
...
const fetchJson = async (endpoint, options = {}) => {
  ...
  const response = await fetch(url, {
    ...
    body: isFormData(body) ? body : body && typeof body === 'object' ? JSON.stringify(body) : body,
    credentials: 'include',
    ...rest,
  });
...
const auth = {
  ...
  verifyTwoFactor: async (challengeId, code) => {
    const data = await fetchJson('/auth/two-factor/verify', { method: 'POST', body: { challengeId, code } });
    if (data?.accessToken) setAuthToken(data.accessToken);
    if (data?.refreshToken) setRefreshToken(data.refreshToken);
    return data;
  },
  updateProfile: async (data) => fetchJson('/auth/profile', { method: 'PATCH', body: data }),
  changePassword: async (currentPassword, newPassword) =>
    fetchJson('/auth/password', { method: 'PATCH', body: { currentPassword, newPassword } }),
  requestEnableTwoFactor: async () => fetchJson('/auth/two-factor/enable/request', { method: 'POST' }),
  confirmEnableTwoFactor: async (code) => fetchJson('/auth/two-factor/enable/confirm', { method: 'POST', body: { code } }),
  requestDisableTwoFactor: async () => fetchJson('/auth/two-factor/disable/request', { method: 'POST' }),
  confirmDisableTwoFactor: async (code) => fetchJson('/auth/two-factor/disable/confirm', { method: 'POST', body: { code } }),
  register: async (data) => {
  ...
  resetPassword: async ({ email, code, newPassword }) => {
    return fetchJson('/auth/reset-password', { method: 'POST', body: { email, code, newPassword } });
  },
